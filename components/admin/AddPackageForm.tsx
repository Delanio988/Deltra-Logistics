...
export default function AddPackageForm({ onSuccess }: AddPackageFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-white/8 bg-navy-900 p-8 shadow-card">
      <h2 className="text-xl font-bold text-white">Add a package</h2>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label htmlFor={accountId} className="text-xs font-semibold uppercase tracking-widest text-white/50">
            Customer
          </label>
          <select
            id={accountId}
          ...
          >
            {CUSTOMERS.map((c) => (
              <option key={c.accountCode} value={c.accountCode} className="bg-navy-900 text-white">
        ...
        <div>
          <label htmlFor={trackingId} className="text-xs font-semibold uppercase tracking-widest text-white/50">
            Tracking number
          </label>
          <input
            id={trackingId}
        ...
        <div>
          <label htmlFor={merchantId} className="text-xs font-semibold uppercase tracking-widest text-white/50">
            Merchant / courier
          </label>
          <input
            id={merchantId}
        ...
        <div className="sm:col-span-2">
          <label htmlFor={descriptionId} className="text-xs font-semibold uppercase tracking-widest text-white/50">
            Description
          </label>
          <input
            id={descriptionId}
        ...
        <div>
          <label htmlFor={dateId} className="text-xs font-semibold uppercase tracking-widest text-white/50">
            Date received
          </label>
          <input
            id={dateId}
            ...
            className="mt-2 w-full rounded-full border border-white/15 bg-white/5 px-5 py-3 text-sm text-white outline-none transition-colors focus:border-accent [color-scheme:dark]"
          />
        </div>

        <div className="sm:col-span-2">
          <label htmlFor={statusId} className="text-xs font-semibold uppercase tracking-widest text-white/50">
            Initial status
          </label>
          <select
            id={statusId}
      ...
      <div className="mt-6 flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-5 py-4">
        <span className="text-sm font-medium text-white/70">Shipping cost (auto-calculated)</span>
