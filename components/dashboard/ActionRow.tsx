...
export default function ActionRow({ icon, label, sublabel, badge, onClick }: ActionRowProps) {
  return (
    <button
      ...
      className="flex w-full items-center gap-4 px-3 py-3 text-left transition-colors hover:bg-fg/5"
    >
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-fg/5 text-accent" aria-hidden>
        {icon}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-semibold text-fg">{label}</span>
        <span className="block truncate text-xs text-fg/50">{sublabel}</span>
      </span>
      {typeof badge === "number" && (
