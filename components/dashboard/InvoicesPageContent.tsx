...
import {
  removeInvoiceFile as removeInvoiceFileAction,
  replaceInvoiceFileInInvoice as replaceInvoiceFileInInvoiceAction,
  restoreInvoice as restoreInvoiceAction,
  withdrawInvoice as withdrawInvoiceAction,
...
export default function InvoicesPageContent({ packages, invoices }: { packages: Package[]; invoices: Invoice[] }) {
  ...
  const handleConfirm = async () => {
    ...
    if (confirmAction.type === "remove-file") {
      const result = await removeInvoiceFileAction({
      ...
      const result = await withdrawInvoiceAction({ invoiceId: confirmAction.invoice.id });
      if (result.success) {
        const removed = result.invoice;
        showToast(`Withdrew your invoice submission for ${confirmAction.pkg.trackingNumber}.`, {
          label: "Undo",
          onClick: async () => {
            const restoreResult = await restoreInvoiceAction({ invoice: removed });
  ...
  const handleReplaceSelected = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const uploaded = await replaceInvoiceFile(oldFile ?? {}, file);
    const result = await replaceInvoiceFileInInvoiceAction({
      invoiceId: target.invoice.id,
      oldFileId: target.fileId,
      newFile: uploaded,
    });
                ...
                return (
                  <li key={invoice.id} className="rounded-2xl border border-fg/8 bg-surface p-6 shadow-card">
                        ...
                        >
                          ...
                          >
                            {file.url && isThumbnailableImage(file.type) ? (
                              // eslint-disable-next-line @next/next/no-img-element -- transient blob/signed-url thumbnail
                    ...
                    {!invoice.files.some((f) => f.url) && (
                      <p className="mt-2 text-xs text-fg/35">Preview unavailable — the signed link may have expired, reload to refresh it.</p>
                    )}
