...
export default function StatsBar() {
  return (
    <section id="stats" aria-label="Deltra Logistics by the numbers" className="bg-bg py-20 text-fg">
        ...
        <dl className="grid grid-cols-2 gap-10 lg:grid-cols-4 lg:gap-6">
          {STATS.map((stat, i) => (
            <ScrollReveal key={stat.label} index={i} direction="up" className="text-center lg:text-left">
              ...
              <dt className="mt-2 text-sm font-medium text-fg/60">{stat.label}</dt>
            </ScrollReveal>
          ))}
        </dl>
      </div>
